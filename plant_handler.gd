...
func new_chunk_handler(new_chunk):
	if WorldManager.get_tile(new_chunk, Vector2i(0, 0)) != null:
		...
		for tile in tiles_to_update:
			...
			var tile_data = WorldManager.get_tile(current_chunk, tile).plant
...
func wither(pos, type, current_chunk):
	if WorldManager.get_tile(current_chunk, pos ).plant.plant_type == "flower":
		print("erased")
		VariablesManager.flower_tiles[current_chunk].erase(pos)
	if WorldManager.get_tile(current_chunk, pos ).plant.plant_type == "plant":
		print("erased")
		VariablesManager.plant_tiles[current_chunk].erase(pos)
	WorldManager.get_tile(current_chunk, pos ).plant = null
...
func try_spread(plant_name, pos, current_chunk):
	...
	for dir in directions:
		var new_pos = pos + dir
		if WorldManager.get_tile(current_chunk, new_pos) == null:
			continue
		if WorldManager.get_tile(current_chunk, new_pos).plant == null and WorldManager.get_tile(current_chunk, new_pos).tree == null and WorldManager.get_tile(current_chunk, new_pos ).object == null:
			...
			WorldManager.get_tile(current_chunk, new_pos ).plant = plant_data
			break
func tilemap_updater(current_chunk):
	...
	for tile in flower_tiles:
		tile_map.set_cell(1, tile, 4, WorldManager.get_tile(current_chunk, tile).plant.atlas_coords)
	for tile in plant_tiles:
		tile_map.set_cell(1, tile, 3, WorldManager.get_tile(current_chunk, tile).plant.atlas_coords)
